...
/*   Updated: 2023/10/30 09:52:54 by rel-isma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	imgs(t_cub *cub)
{
	...
	cub->img.sprits[5] = mlx_xpm_file_to_image(cub->mlx, "./sprite/frame04.xpm",
			&cub->sprite_img.bits_per_pixel, &cub->sprite_img.line_length);
	if (!cub->img.sprits[0] || !cub->img.sprits[1]
		|| !cub->img.sprits[2] || !cub->img.sprits[3]
...
void	init_img(t_cub *cub)
{
	...
	cub->img.sprits[11] = mlx_xpm_file_to_image(cub->mlx,
			"./sprite/frame10.xpm",
			&cub->sprite_img.bits_per_pixel, &cub->sprite_img.line_length);
	if (!cub->img.sprits[6] || !cub->img.sprits[7]
		|| !cub->img.sprits[8] || !cub->img.sprits[9]
		|| !cub->img.sprits[10] || !cub->img.sprits[11])
		printerror_message("error");
}
